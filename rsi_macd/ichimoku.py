...
class Ichimoku():
    ...
    def run(self):
        ...
        dates_ext_df = pd.DataFrame({"timestamp": dates_ext})
        dates_ext_df.index = dates_ext # also update the df index
        ohcl_df = ohcl_df.append(dates_ext_df)

        # Tenkan 
        tenkan_sen_high = ohcl_df['high_price'].rolling( window=tenkan_window ).max()
        tenkan_sen_low = ohcl_df['low_price'].rolling( window=tenkan_window ).min()
        ohcl_df['tenkan_sen'] = (tenkan_sen_high + tenkan_sen_low) /2
        # Kijun 
        kijun_sen_high = ohcl_df['high_price'].rolling( window=kijun_window ).max()
        kijun_sen_low = ohcl_df['low_price'].rolling( window=kijun_window ).min()
        ...
        # Senkou Span B 
        senkou_span_b_high = ohcl_df['high_price'].rolling( window=senkou_span_b_window ).max()
        senkou_span_b_low = ohcl_df['low_price'].rolling( window=senkou_span_b_window ).min()
        ohcl_df['senkou_span_b'] = ((senkou_span_b_high + senkou_span_b_low) / 2).shift(cloud_displacement)
        # Chikou
        ohcl_df['chikou_span'] = ohcl_df['trade_price'].shift(chikou_shift)
    ...
    def plot_candlesticks(self, fig, ax, view_limit=100):
        # plot candlesticks
        candlesticks_df = self.ohcl_df.loc[:, ['timestamp','opening_price','high_price','low_price', 'trade_price']]
